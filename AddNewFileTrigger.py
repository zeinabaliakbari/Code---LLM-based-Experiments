...
def run_function( file_path):
        parent_directory = English_REMOTE_DIR  # C:\Users\zeina\processedDir
        ...
        output_path = os.path.join(processedDir,directory ,os.path.splitext(file_name)[0] + '_processed.pdf')
        first_intro_page = find_second_introduction(file_path)
        if first_intro_page==None:
            encryptedFile_path=os.path.join(encrypted_Dir,directory ,os.path.splitext(file_name)[0] + 'encrypted.pdf')

            shutil.copy(file_path, encryptedFile_path) # encrypter
